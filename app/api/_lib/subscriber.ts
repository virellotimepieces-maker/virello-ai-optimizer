...
const usageState = new Map<
  string,
  { periodStart: number; count: number }
>();
...
export async function getCheckoutSubscription(
  ...
  const checkoutSession = await stripeRequest<any>(
...
function updateUsageState(
  ...
  const usageKey = `${subscription.subscriptionId}:${subscription.currentPeriodStart}`;
...
export async function authorizeSubscriberForAI(
  ...
  const cookieSecret = getCookieSecret(stripeSecret);
  ...
  const subscriber = decodeSubscriberPayload(
    encoded,
    cookieSecret
  );
  ...
  const refreshedSubscription = normalizeSubscription(
  ...
  const payload: SubscriberPayload = {
    ...
    subscriptionId:
      refreshedSubscription.subscriptionId,
    customerId: refreshedSubscription.customerId,
  ...
  return {
    cookieValue: encodeSubscriberPayload(
      payload,
      cookieSecret
    ),
    usage,
...
export function setSubscriberCookie(
  ...
  response.cookies.set(SUBSCRIBER_COOKIE, cookieValue, {
    httpOnly: true,
...
export function clearSubscriberCookie(
  ...
  response.cookies.set(SUBSCRIBER_COOKIE, "", {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
    path: "/",
...
export function buildSubscriberCookieValue(
  ...
  const stripeSecret = getStripeSecret();
  const secret = getCookieSecret(stripeSecret);

  ensureActiveStatus(subscription.status);
  ...
  return encodeSubscriberPayload(payload, secret);
}
